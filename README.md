...
- **토큰 저장**: HttpOnly 쿠키 + localStorage 플래그
- **토큰 전송**: 쿠키 자동 전송 + Authorization 헤더 (이중 보안)
- **토큰 갱신**: 만료 시 자동 리디렉션 + 세션 유지
- **CSRF 보호**: 토큰 기반 인증으로 CSRF 방지
- **XSS 방지**: HttpOnly 쿠키 사용으로 JavaScript 접근 차단
- **서버 사이드 인증**: 페이지 로드 시 서버에서 토큰 검증
